...
产物: dist/ThesisForge-v0.2.0-win64-offline.zip
安全约定：
...
DIST = ROOT / "dist"
PKG = DIST / "ThesisForge-v0.2.0-win64"
RT = PKG / "runtime"
...
def main():
    ...
    (PKG / "启动毕设工坊.bat").write_bytes(
        ("@echo off\r\nchcp 65001 >nul\r\ncd /d %~dp0\r\n"
         ...
         "runtime\\python.exe -m uvicorn app.main:app --host 127.0.0.1 --port 8765\r\n"
         "pause\r\n").encode("utf-8"))
    ...
    (PKG / "使用说明.txt").write_text(
        """毕设工坊 ThesisForge v0.2.0 — Windows 离线整合包
================================================

【快速开始】
1. 双击「启动毕设工坊.bat」，浏览器会自动打开 http://127.0.0.1:8765
...
- 端口 8765 被占用时，编辑启动 bat 更换端口
""", encoding="utf-8")

    # 5. 压缩
    step("压缩为 zip ...")
    zpath = DIST / "ThesisForge-v0.2.0-win64-offline.zip"
    if zpath.exists():
        zpath.unlink()
