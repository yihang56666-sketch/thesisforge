...
- **🚀 模型训练**：27 个模型覆盖表格分类/回归、文本分类、图像分类、时间序列预测、目标检测、实例分割；表单化超参数（优化器、学习率调度、早停、梯度裁剪等）；后台子进程训练，实时日志与曲线；自动使用 GPU（CUDA），无卡自动回退 CPU。
...
到 [Releases](../../releases) 下载 `ThesisForge-v0.4.3-win64-offline.zip`（无需安装 Python，解压即用）：
...
不想解压也可以下载独立单文件版 `ThesisForge-v0.4.3-win-x64.exe`，双击即用（自带 Python 与表格/文本/报告全部依赖；为控制体积不含 PyTorch，图像训练请用离线整合包）。
