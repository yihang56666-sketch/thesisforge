...
到 [Releases](../../releases) 下载 `ThesisForge-v0.5.0-win64-offline.zip`（无需安装 Python，解压即用）：
...
不想解压也可以下载独立单文件版 `ThesisForge-v0.5.0-win-x64.exe`，双击即用（自带 Python 与表格/文本/报告全部依赖；为控制体积不含 PyTorch，图像训练请用离线整合包）。
